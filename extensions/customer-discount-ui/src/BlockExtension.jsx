...
const FABER_STANDARD_PRESET_DISCOUNTS = [
...
function CustomerDiscountBlock() {
  ...
  async function applyFaberStandardPreset() {
    setSaving(true);
    setError("");
    setSuccess("");

    try {
      const cleanDiscounts = cleanB2BDiscounts(FABER_STANDARD_PRESET_DISCOUNTS);

      const nextCustomerTags = [
        ...originalCustomerTags.filter((tag) => !isManagedB2BDiscount(tag)),
        ...cleanDiscounts,
      ];
      ...
      setOriginalDiscounts(updatedDiscounts);
      setOriginalCustomerTags(updatedTags);
      setSuccess("Preset standard Faber applicato su metafield e tag.");
    } catch (err) {
      setError(err?.message || "Unable to apply Faber standard preset.");
    } finally {
      setSaving(false);
    }
  }
        ...
        {loading ? (
          ...
          <>
            ...
            <s-stack gap="none">
              <s-button
                disabled={saving}
                onClick={applyFaberStandardPreset}
              >
                Applica preset standard Faber
              </s-button>
              <s-text>
                Applica in automatico: Slot 20%, Pro Inox 20%+5%,
                Pro Essential 20%+5%, Pro Deluxe 2.0 20%,
                Pro Deluxe 3.0 20%+10%, Agenta 20%.
              </s-text>
            </s-stack>
